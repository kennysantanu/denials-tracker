...
export const load: PageServerLoad = async ({ locals, parent, url, request }) => {
	...
	const { data: logs, count, error: dbError } = await getAuditLogs(locals.supabase, {
		page,
		pageSize,
	...
	logAudit(locals.supabase, user.id, 'view', 'audit_log', null, { page, filters: { userId, action, resourceType, startDate, endDate } }, request);
