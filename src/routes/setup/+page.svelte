...
<div class="flex min-h-screen items-center justify-center bg-surface-50 px-4">
	<div class="w-full max-w-md space-y-6 rounded-xl bg-white p-8 shadow-lg">
		<div class="text-center">
			...
			<p class="mt-2 text-sm text-surface-500">Create your administrator account</p>
		</div>
		...
		<form method="POST" use:enhance class="space-y-4">
			...
			<label class="block">
				<span class="text-sm font-medium text-surface-700">Confirm Password</span>
				<input
					type="password"
				...
				{#if $errors.confirmPassword}
					<p class="mt-1 text-sm text-error-500">{$errors.confirmPassword}</p>
				{/if}
			</label>
